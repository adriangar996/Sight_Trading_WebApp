...
def helpView(request):
    ...
    #             if not StockPortfolio.objects.filter(user=user_id, symbol=new_stock.upper()):  # stock not already in portfolio

    #                 logger.info('Adding ' + new_stock.upper() + ' to stock portfolio')

    #                 try:  # try to add stock to portfolio
    #                     stock_object = Share(new_stock.upper())
    #                     new_stock_name = stock_object.get_name()
    #                     new_stock_price = stock_object.get_price()
    #                     new_stock_change = stock_object.get_percent_change()
    #                     shares_owned = form.cleaned_data['stocks_bought']
    #                     buying_price = form.cleaned_data['buying_price']
    ...
    #                     stock = StockPortfolio.objects.filter(user=user_id, symbol=new_stock.upper())
    #                     stocks = stock.shares_owned
    #                     name = stock.name
    #                     bprice = stock.buying_price
    #                     price = stock.price
    ...
    #     if StockPortfolio.objects.filter(user=user_id, symbol=symbol).count() > 0:  # if inserted stock is in portfolio

    #         logger.info('Removing ' + symbol + ' from stock portfolio')

    #         StockPortfolio.objects.filter(user=user_id, symbol=symbol).delete()
    #         stock_list = StockPortfolio.objects.order_by('price')[:10]
